...
export class UpdateMissionComponent implements OnInit, OnDestroy {
  ...
  fetchDetail(id: any) {
    const fetchDetailSubscription = this._service.missionDetailById(id).subscribe((data: any) => {
      ...
      if (this.editData.missionImages) {
        ...
        this.imageListArray = [] // Clear existing array
  ...
  onSelectedImage(event: any) {
    ...
    if (files && files.length > 0) {
      ...
      if (this.imageListArray.length + files.length > 6) {
        return this._toast.error({ detail: "ERROR", summary: `You can only add ${6 - this.imageListArray.length} more image(s).`, duration: APP_CONFIG.toastDuration })
      }
  ...
  onRemoveImage(index: number) {
    if (index >= 0 && index < this.imageListArray.length) {
      this.imageListArray.splice(index, 1)
      
      // If you want to reset the file input when all images are removed
      if (this.imageListArray.length === 0) {
        this.isFileUpload = false
        this.editMissionForm.get('missionImages')?.setValue('')
        this.formData = new FormData()
      }
      
      // Show success message
      this._toast.success({ detail: "SUCCESS", summary: "Image removed successfully.", duration: APP_CONFIG.toastDuration })
    }
  }
